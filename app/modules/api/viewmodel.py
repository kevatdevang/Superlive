...
class ApiViewModel:
    ...
    async def search_users(self, search_query, token=None, client=None, base_url=None):
        if client is None:
            client = SuperliveClient.get_client()
        ...
        # Customize client params for search context
        client_params = self._get_client_params()
        client_params["source_url"] = "https://superlive.chat/search"
        ...
        return await self._make_request(
            ...
            headers=headers,
            json=payload,
            error_context="Search users failed",
            base_url=base_url
        )

    async def get_discover(self, next_cursor=None, type_val=6, client=None, base_url=None):
        if client is None:
            client = SuperliveClient.get_client()

        # No token needed typically for public discover, but we use client headers which might have device-id
        
        client_params = self._get_client_params()
        client_params["source_url"] = "https://superlive.chat/discover"
        ...
        return await self._make_request(
            ...
            client,
            json=payload,
            error_context="Get discover failed",
            base_url=base_url
        )
