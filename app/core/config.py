...
class Config:
    ...
    if os.path.exists(_proxy_file):
        try:
            ...
            if PROXIES:
                print(f"[INFO] Loaded {len(PROXIES)} proxies from proxy.txt")
        except Exception as e:
            print(f"[WARN] Failed to load proxy.txt: {e}")
    ...
    MONGO_URI = os.environ.get("MONGO_URI", "mongodb+srv://Rajat:2844@cluster0.gpq2duh.mongodb.net/?appName=Cluster0")
